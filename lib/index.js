...
const init = (elm: HTMLElement, options: DragInitializationOptions) => {
  if (!elm) {
    return;
  }
...
const onBodyMouseDown = (evt: MouseEvent) => {
  onMouseDown(evt, elements.get(activeDraggable));
};

const onBodyMouseUp = (evt: MouseEvent) => {
  onMouseUp(evt, elements.get(activeDraggable));
};

const onBodyMouseMove = (evt: MouseEvent) => {
